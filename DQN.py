...
class DQN:
    def __init__(self, args, env):
        ...
        self._behavior_net = Net(env).to(args.device) # Main network
        self._target_net = Net(env).to(args.device) # update using main network's parameters after a while
    ...
    def update(self, total_steps):
        if total_steps % self.freq == 0:    # self.freq = 4
            self._update_behavior_network(self.gamma)
        if total_steps % self.target_freq == 0:  # self.target_freq = 100
